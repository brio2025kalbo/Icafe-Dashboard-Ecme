...
export async function apiGetCafeGames(
    cafeId: string,
): Promise<IcafeGame[]> {
    const cafe = getCafeById(cafeId)
    const allGames: IcafeGame[] = []
    let page = 1
    let totalPages = 1

    do {
        const response = await icafeAxios.get<IcafeGamesResponse>(
            `/cafe/${cafe.cafeId}/games`,
            {
                params: { page },
                headers: { Authorization: `Bearer ${cafe.apiKey}` },
            },
...
export async function apiGetGamePosterBlob(
    cafeId: string,
    gameId: number,
): Promise<string | null> {
    try {
        ...
        const response = await icafeAxios.get(
            `/cafe/${cafe.cafeId}/games/${gameId}/poster`,
            {
                headers: { Authorization: `Bearer ${cafe.apiKey}` },
